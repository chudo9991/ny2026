...
### Вариант 1: Запуск через Docker с SSL (рекомендуется)
...
# Запустите инициализацию SSL (замените на ваш домен и email)
...
**Важно:** Для работы SSL нужен:
- Домен, указывающий на IP вашего сервера
- Порты 80 и 443, открытые в файрволе

### Вариант 1b: Запуск без SSL (для локального использования)

Если вам не нужен SSL, отредактируйте `docker-compose.yml`:
...
services:
  ar-app:
    build: .
    ports:
      - "9000:80"  # Измените обратно на 9000:80
    # Удалите volumes и environment
```
